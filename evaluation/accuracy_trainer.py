# evaluation/accuracy_trainer.py
import math
...
class AccuracyTrainer:
    """
    Differentiable multiple-choice **cross-entropy** trainer with internal dataset loading.
      - Call `num_batches(task, ...)`
      - Call `batch_loss_by_index(..., task=..., batch_idx=..., batch_size=...)`
    Implementation mirrors EntropyTrainer's memory-friendly batching:
      we score each choice by (mean|sum) log-probs over its tokens, then
      compute CE = -log softmax(scores)[answer_idx].
    """
    ...
    # ---------- core CE loss for one batch (memory-friendly) ----------
    def _cross_entropy_batch_loss(self, model_callable, tokenizer, samples, *, max_len: int, length_norm: bool):
        """
        Build a single multiple-choice batch and compute **cross-entropy**:
          For each sample:
            scores[c] = sum/mean log p(choice_tokens | context)
            CE_i = -log softmax(scores)[answer_idx]
          Return mean CE over samples.
        """
        device = self.device
        ...
        # Keep only needed logits to save memory.
        ...
        raw_logits = out.logits  # [B, K, V] if trimmed, else [B, T, V]
        ...
        answer_idx = [int(s["answer_idx"]) for s in samples]
        ...
        for j, (s_idx, c_idx) in enumerate(owners):
            ...
            if kept_c <= 0 or T_kept <= 0 or kept_c > T_kept:
                score = logits.new_tensor([-1e30])  # effectively impossible choice
            else:
        ...
        # Cross-entropy over choices (using the gold answer)
        losses = []
        for i, scores in enumerate(per_sample_scores):
            log_probs = F.log_softmax(scores, dim=-1)
            gold = answer_idx[i]
            losses.append(-log_probs[gold])
        loss = torch.stack(losses).mean()

        # Cleanup local refs
        try:
            del seqs, kept_choice_lens, owners, batch_ids, attn_mask, logits, targets, per_sample_scores, losses
        except Exception:
            pass
    ...
    def batch_loss_by_index(
        ...
        *,
        ...
        """
        Load/cache the dataset internally, slice the requested batch, and compute **cross-entropy** loss.
        ...
        loss = self._cross_entropy_batch_loss(
